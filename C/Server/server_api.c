...
unsigned char* get_frame(int socket, int* size){
    ...
    *size = pack_size;
    return get_message(socket, pack_size);
}

// std::vector<unsigned char> decode_frame(unsigned char *frame, unsigned int size){
...
// std::vector<unsigned char> get_frame(int socket, bool request=false){
//     unsigned int pack_size;
